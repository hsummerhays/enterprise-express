...
export function setupRoutes(app: Express): void {
	...
	registerAuthRoutes(authRouter, container.resolve<AuthController>(AuthController));
	app.use("/auth", authRouter);

	const healthRouter = Router();
	registerHealthRoutes(healthRouter, container.resolve<HealthController>(HealthController));
	app.use("/health", healthRouter);

	const userRouter = Router();
	registerUserRoutes(userRouter, container.resolve<UserController>(UserController));
	app.use("/users", userRouter);

	const sampleDataRouter = Router();
	registerSampleDataRoutes(sampleDataRouter, container.resolve<SampleDataController>(SampleDataController));
